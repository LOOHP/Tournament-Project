...
public class FunctionSendFinalReport {
	...
	public static String getReport(Player first, Player runner, Player runner2, Player runner3) {
		...
		output = output + TournamentServer.getInstance().getLang().get("Functions.ReportGenerator.Winner").replace("%s", first.getName()) + "\n";
		output = output + "" + "\n";
		output = output + TournamentServer.getInstance().getLang().get("Functions.ReportGenerator.RunnerUp").replace("%s", runner.getName()) + "\n";
		output = output + "" + "\n";
		if (runner2 != null) {
			output = output + TournamentServer.getInstance().getLang().get("Functions.ReportGenerator.RunnerUp2").replace("%s", runner2.getName() + (runner3 != null ? ", " : "") + (runner3 != null ? runner3.getName() : "")) + "\n";
		}
